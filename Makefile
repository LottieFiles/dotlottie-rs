...
RUNTIME_FFI_DYLIB := libdlplayer.dylib
...
define UNIFFI_BINDINGS_BUILD
	cargo +nightly run \
		...
		--out-dir $(RUNTIME_FFI)/uniffi-bindings/$(BINDINGS_LANGUAGE)
endef
...
$$($1_THORVG_DEP_BUILD_DIR)/../$(THORVG_CROSS_FILE): SYSROOT := $(ANDROID_NDK_HOME)/toolchains/llvm/prebuilt/$(ANDROID_BUILD_PLATFORM)/sysroot/usr/lib/$$($1_ARCH)/$(ANDROID_API_VERSION)
$$($1_THORVG_DEP_BUILD_DIR)/../$(THORVG_CROSS_FILE): CPP := $(ANDROID_NDK_HOME)/toolchains/llvm/prebuilt/$(ANDROID_BUILD_PLATFORM)/bin/$$($1_ARCH)$(ANDROID_API_VERSION)-clang++
...
$$($1_THORVG_DEP_BUILD_DIR)/../$(THORVG_CROSS_FILE): export OUTPUT_FILE := $$(ANDROID_CROSS_FILE)
...
# Create cross file for thorvg
$$($1_THORVG_DEP_BUILD_DIR)/../$(THORVG_CROSS_FILE): ARCH := $$($1_ABI)
$$($1_THORVG_DEP_BUILD_DIR)/../$(THORVG_CROSS_FILE): PLATFORM := $$($1_PLATFORM)
$$($1_THORVG_DEP_BUILD_DIR)/../$(THORVG_CROSS_FILE): SDK := $$($1_SDK)
$$($1_THORVG_DEP_BUILD_DIR)/../$(THORVG_CROSS_FILE): SUBSYSTEM := $$($1_SUBSYSTEM)
$$($1_THORVG_DEP_BUILD_DIR)/../$(THORVG_CROSS_FILE): CPU_FAMILY := $$($1_CPU_FAMILY)
$$($1_THORVG_DEP_BUILD_DIR)/../$(THORVG_CROSS_FILE): CPU := $$($1_CPU)
$$($1_THORVG_DEP_BUILD_DIR)/../$(THORVG_CROSS_FILE): DEP_BUILD_DIR := $$($1_THORVG_DEP_BUILD_DIR)
$$($1_THORVG_DEP_BUILD_DIR)/../$(THORVG_CROSS_FILE): export OUTPUT_FILE := $$(APPLE_CROSS_FILE)
$$($1_THORVG_DEP_BUILD_DIR)/../$(THORVG_CROSS_FILE):
	$$(CREATE_OUTPUT_FILE)
endef
...
$$($1_THORVG_DEP_BUILD_DIR)/$(THORVG_NINJA_BUILD_FILE): export PKG_CONFIG_PATH := $(PWD)/$$($1_DEPS_LIB_DIR)/pkgconfig:$(PWD)/$$($1_DEPS_LIB64_DIR)/pkgconfig
...
$(THORVG_LOCAL_ARCH_BUILD_DIR)/$(THORVG_NINJA_BUILD_FILE): export PKG_CONFIG_PATH := $(PWD)/$(LOCAL_ARCH_LIB_DIR)/pkgconfig:$(PWD)/$(LOCAL_ARCH_LIB64_DIR)
...
help:
	...
	@echo
	@echo "$(GREEN)*************************************************************************************************$(NC)"
	@echo "$(GREEN)NOTE$(NC): If you are a $(GREEN)mac$(NC) user, run $(YELLOW)make mac-setup$(NC) the very first time before performing any builds."
