...
impl Animation for TvgAnimation {
    ...
    fn tween(
        ...
        #[cfg(feature = "thorvg-v1")]
        {
            ...
            self.tween_state = Some(TweenState {
                start_time: _duration.is_some() ? Some(Instant::now()) : None,
                from,
                to: _to,
                duration: _duration,
                easing: _easing,
    ...
    fn tween_update(&mut self, _given_progress: Option<f32>) -> Result<bool, TvgError> {
        #[cfg(feature = "thorvg-v1")]
        {
            if self.tween_state.is_some() && self.tween_state.unwrap().duration.is_none() {
                if _given_progress.is_none() {
                    return Err(TvgError::InvalidArgument);
                }

                let tween_state = self.tween_state.unwrap();
