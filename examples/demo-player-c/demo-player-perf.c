...
void usage(char *app) {
  ...
  fprintf(
      stderr,
      "  num-animations: number of animations to display (1-%d, default: 4)\n",
      MAX_ANIMATIONS);
  exit(1);
}
...
#ifdef __APPLE__
  // On macOS, use task_info to get current memory usage (matches Activity
  // Monitor)
...
void calculate_grid_layout(int num_animations, int window_width,
                           int window_height, int *cols, int *rows,
                           int *anim_width, int *anim_height) {
...
void update_metrics(PerformanceMetrics *metrics, double frame_time_ms) {
  ...
  metrics->avg_frame_time_ms =
      metrics->total_render_time_ms / metrics->frame_count;
...
void render_metrics(SDL_Renderer *renderer, TTF_Font *font,
                    PerformanceMetrics *metrics, int num_animations,
                    int window_width) {
  if (!font)
    return;
  ...
  // Frame Time
  snprintf(text, sizeof(text), "Frame Time: %.2f ms",
           metrics->avg_frame_time_ms);
  ...
  // Resolution
  snprintf(
      text, sizeof(text), "Resolution: %dx%d", window_width,
...
int main(int argc, char **argv) {
  ...
  calculate_grid_layout(num_animations, window_width, window_height, &cols,
                        &rows, &anim_width, &anim_height);

  // Create window
  window =
      SDL_CreateWindow("DotLottie Performance Test", SDL_WINDOWPOS_UNDEFINED,
  ...
  if (SDL_GetCurrentDisplayMode(display_index, &display_mode) != 0) {
    fprintf(stderr,
            "Warning: Could not get display mode, defaulting to 60Hz\n");
  ...
  if (!font) {
    fprintf(stderr, "Warning: Could not load font, performance metrics will "
                    "not be displayed\n");
  }
  ...
  for (int i = 0; i < num_animations; i++) {
    ...
    animations[i].texture =
        SDL_CreateTexture(renderer, SDL_PIXELFORMAT_BGRA32,
  ...
  while (running) {
      ...
      if (e.type == SDL_QUIT) {
        ...
        if (e.window.event == SDL_WINDOWEVENT_SIZE_CHANGED) {
          ...
          for (int i = 0; i < num_animations; i++) {
            ...
            ret = dotlottie_load_animation_path(
                animations[i].player, animation_path, anim_width, anim_height);
            if (ret != DOTLOTTIE_SUCCESS) {
            ...
            ret = dotlottie_buffer_ptr(animations[i].player,
                                       &animations[i].buffer);
            if (ret != DOTLOTTIE_SUCCESS) {
              fprintf(stderr, "Could not get buffer for animation %d\n", i);
              running = 0;
            ...
            animations[i].texture = SDL_CreateTexture(
    ...
    for (int i = 0; i < num_animations; i++) {
      ...
      SDL_Rect dst_rect = {animations[i].x, animations[i].y,
