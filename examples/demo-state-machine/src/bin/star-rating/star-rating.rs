...
fn main() {
    ...
    let mut markers = File::open("src/bin/star-rating/star-rating.lottie").expect("no file found");
    let metadatamarkers =
        fs::metadata("src/bin/star-rating/star-rating.lottie").expect("unable to read metadata");
    ...
    let message: String = fs::read_to_string("./src/bin/star-rating/star_hover.json").unwrap();
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        if left_down && !clicked {
            ...
            let _m = p.post_event(&pointer_event);

            println!("{} {}", mx, my);
        } else if !left_down && clicked {
            let mut mx = 0.0;
            let mut my = 0.0;
            window.get_mouse_pos(MouseMode::Clamp).map(|mouse| {
                mx = mouse.0;
